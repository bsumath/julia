...
public class ComplexCalculator extends JFrame {
    ...
    public ComplexCalculator() {
        ...
        labelX = new JLabel ("When using the calculator number buttons, the TAB button ");
        commentPanel2.add(labelX);
        CalculatorFrame.add(commentPanel2);
        Panel commentPanel3 = new Panel();
        labelY = new JLabel ("         must be used to toggle between the input boxes.            ");
    ...
    public boolean action(Event evt, Object arg) {
    	try {
    		...
    		//operations
    				...
    				| ("/".equals(arg))| ("=".equals(arg)) | ("Sqrt".equals(arg))
    				| ("Curt".equals(arg)) | ("z^2".equals(arg)) | ("z^3".equals(arg))) {
    			first = firstfield.getText();
    			second = secondfield.getText();
    			seed2 = new ComplexNumber((Double.valueOf(first)).doubleValue(), (Double.valueOf(second)).doubleValue());
        	...
        	JOptionPane.showMessageDialog(ComplexCalculator.this, "The input must be" + 
					 " number with no commas.", "Number Format Error", JOptionPane.ERROR_MESSAGE); 
		}
        return true;	
    }
    // Calculation
    private ComplexNumber Calculation(String sOperator, ComplexNumber dReg1, ComplexNumber dReg2)
    {
    	ComplexNumber[] result = new ComplexNumber[3];
             if ("+".equals(sOperator)) dReg1 = dReg1.add(dReg2);
        else if ("-".equals(sOperator)) dReg1 = dReg1.subtract(dReg2);
        else if ("*".equals(sOperator)) dReg1 = dReg1.multiply(dReg2);
        else if ("/".equals(sOperator)) dReg1 = dReg1.divide(dReg2);
        ...
        else if ("z^2".equals(sOperator)) dReg1 = dReg1.multiply(dReg1);
        else if ("z^3".equals(sOperator)) dReg1 = dReg1.multiply(dReg1.multiply(dReg1));
        else dReg1 = dReg2;
        return dReg1;
    }
}
