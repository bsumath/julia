 ...
 * 
 * <h3>Description</h3>
 * <p>This class represents a complex number of the form
 * x + bi where x and y are real numbers and i is the square
 * root of -1.</p>
 * <p>Methods are provided for simple mathematical manipulation
 * of complex numbers, as well as get methods.</p>
 *
 */
public class ComplexNumber {
	...
	private double y;
	/**
	 * The constant alpha is used in determining the three cube roots 
	 * of a number.  The primary cube root does not use alpha, but the 
	 * secondary is alpha times the primary and the third is alpha squared 
	 * times the primary.
	 */
	private static final ComplexNumber ALPHA = new ComplexNumber(
			Math.cos(2*Math.PI/3), Math.sin(2*Math.PI/3));
	
	/**
	 * The default constructor creates a new complex number of the form:
	 * 0 + 0 * i.
	 */
	...
	/**
	 * This constructor creates a complex number of the form: 
	 * xValue + yValue * i.
	 * @param xValue
	 * @param yValue
	 */
	...
	/**
	 * @return y:  When multiplied by i, the imaginary part of the 
	 * complex number.
	 */
	public double getY() {
		return y;
	...
	/**
	 * Checks to see if this complex number is of the form 0 + 0 * i.  
	 * The x and y values are doubles, so the checking this method does is 
	 * highly inexact.  It is meant solely for use in checking user input.  
	 * Do not use on any complex number that has been mathematically changed
	 * in any way.
	 * @return A boolean indicating whether this complex number is 0 (true) 
	 * or not (false).	 
	 */
	...
	public ComplexNumber add (ComplexNumber a) {
		return new ComplexNumber(x + a.getX(),
				y + a.getY());
	}
	/**
	 ...
	 * @return An array of size 2, with the primary root in position 0 and 
	 * the secondary root in position 1.
	 */
	 ...
	 * secondary root in position 1, and the tertiary root in position 2.
	 */
	public ComplexNumber[] cubeRoot(){
		ComplexNumber[] result = new ComplexNumber[3];
		double xResult = Math.pow(x*x + y*y,(1.0/6.0)) * Math.cos(arg()/3);
		double yResult = Math.pow(x*x + y*y,(1.0/6.0)) * Math.sin(arg()/3);
		ComplexNumber baseResult = new ComplexNumber(xResult, yResult);
		result[0] = baseResult;
	 ...
	 * mathematical argument of a complex number.  This method is sensitive 
	 * to division by zero.  If the complex number is 0, the argument returned
	 * will be zero.
	...
	/**
	 * Provides a string representation of the complex number in the form: 
	 * "{x, y}".  "Computerized Scientific Notation" (the E form) is replaced
	 * with standard scientific notation.
	 */
