...
public class InputPanel extends JPanel implements PropertyChangeListener {
	...
	public InputPanel(Julia f) {
		...
		TextFieldFocusListener iterationListener = new TextFieldFocusListener(iterationsField, parentFrame);
		...
		TextFieldFocusListener skipListener = new TextFieldFocusListener(skipsField, parentFrame);
		...
		TextFieldFocusListener seed1Listener = new TextFieldFocusListener(seedField1, parentFrame);
		...
		TextFieldFocusListener seed2Listener = new TextFieldFocusListener(seedField2, parentFrame);
		...
		for(int i = 0; i<fns.size(); i++) listModel.addElement(fns.elementAt(i));
		
		inputList = new JList(listModel);
		inputList.setSelectionMode
		...
		inputList.setBorder(BorderFactory.createTitledBorder("Input Functions"));
		...
		MouseListener mouseListener = new MouseAdapter() {
			...
			private void checkPopup(MouseEvent e) {
				if(e.isPopupTrigger()) {
					...
					popup.add(new EditFunctionAction
							(parentFrame, listModel, index));
					popup.add(new DeleteAction(parentFrame, 
							listModel, index, Julia.INPUTTYPE));
					popup.add(new CopyFunctionAction
							(parentFrame, listModel, index));
					popup.addSeparator();
					popup.add(new DeleteSelectedAction(parentFrame, Julia.INPUTTYPE));
		...
		buttonPanel2.setBorder(BorderFactory.createTitledBorder("Process Type"));
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if (name.equals("iterations"))iterationsField.setText
			(""+e.getNewValue());
		else if (name.equals("skips"))skipsField.setText
			(""+e.getNewValue());
		else if (name.equals("seed")){ 
			String show = String.valueOf(((ComplexNumber)e.getNewValue()).getX());
			String showShort = show;
			if (show.length()>10)
				showShort =show.substring(0,10);
			seedField1.setText(showShort);
			...
			if(listModel.isEmpty()) processButton.setEnabled(true);
			if(listModel.isEmpty()) postCriticalButton.setEnabled(true);
			if(listModel.isEmpty()) createCompButton.setEnabled(true);
			if(listModel.isEmpty()) createIndButton.setEnabled(true);
			listModel.addElement(e.getNewValue());
			inputList.addSelectionInterval(listModel.size() - 1,
					listModel.size() - 1);
		}
		else if (name.equals("replaceInputFunction"))listModel.set
				(listModel.indexOf(e.getOldValue()), e.getNewValue());
			...
			skipsField.setText
				(""+ s.getSkips());
			seedField1.setText
				(""+ ((ComplexNumber)s.getSeedValue()).getX());
			seedField2.setText
			(""+ ((ComplexNumber)s.getSeedValue()).getY());
			listModel.clear();
	...
	public JPanel createProcessButtonPanel() {
		...
		if(listModel.isEmpty()) postCriticalButton.setEnabled(false);
		...
		if(listModel.isEmpty()) createCompButton.setEnabled(false);
		...
		if(listModel.isEmpty()) createIndButton.setEnabled(false);
		...
		if(listModel.isEmpty()) processButton.setEnabled(false);
