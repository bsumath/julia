...
public class OutputPanel extends JPanel implements PropertyChangeListener{
	...
	public OutputPanel(Julia f) {
		...
		outputList.setSelectionMode
			(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		outputList.setFocusable(false);
		outputList.setBackground(getBackground());
		outputList.setBorder
			(BorderFactory.createTitledBorder("Output Sets"));
		...
		MouseListener mouseListener = new MouseAdapter() {
			...
			private void checkPopup(MouseEvent e) {
				if(e.isPopupTrigger()) {
					...
					popup.add(new ChangeColorAction(parentFrame, 
							listModel, index));
					popup.add(new DeleteAction(parentFrame, 
							listModel, index, Julia.OUTPUTTYPE));
					popup.add(new SaveSetAction(parentFrame, listModel, index));
					popup.addSeparator();
					popup.add(new PropertiesAction
							(parentFrame, listModel, index));
					popup.addSeparator();
					popup.add(new DeleteSelectedAction(parentFrame, Julia.OUTPUTTYPE));
		...
		if(listModel.isEmpty()) inverseButton.setEnabled(false);
		...
		if(listModel.isEmpty()) forwardButton.setEnabled(false);
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (name.equals("addOutputFunction")){
			...
			if(listModel.isEmpty()) inverseButton.setEnabled(true);
			if(listModel.isEmpty()) forwardButton.setEnabled(true);
			listModel.addElement(fn);
			fn.addListener(this);
			outputList.addSelectionInterval(listModel.size() - 1,
					listModel.size() - 1);
		}
